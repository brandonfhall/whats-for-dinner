...
- **AI**: Anthropic Claude (default), OpenAI, or any self-hosted OpenAI-compatible endpoint (e.g. LiteLLM), configurable via `AI_PROVIDER` env var
...
+---------------------------+
             ...
             |
+-----------------------------------------------+
|  AI Provider (external/self-hosted)           |
|  Claude API / OpenAI API / OpenAI-compatible  |
|  Called from /api/ai                          |
+-----------------------------------------------+
```
...
5. **AI Suggestions**: Three modes:
   ...
   - Gym nights are worded as a hard requirement in the prompt (`_build_prompt`), but prompt compliance isn't guaranteed — `_enforce_gym_nights` (`app/routers/ai.py`) is a code-level backstop that runs after `_apply_suggestions` and before commit: any gym night that isn't a `home_cooked` day with an `easy_to_make` meal gets force-swapped to a random `easy_to_make` meal and flagged with an "Auto-adjusted" note. Days with `carry_forward=True` are exempt — that flag is a deliberate per-day user choice (e.g. a standing eat-out night) and isn't overridden. If the library has no `easy_to_make` meal at all, the non-compliant day is left as-is and a warning is logged.
