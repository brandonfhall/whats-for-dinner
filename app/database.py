...
def _seed_demo_data():
    """Seed demo meals and protein quantities when DEMO_MODE is enabled."""
    if not os.getenv("DEMO_MODE", "").lower() in ("true", "1", "yes"):
        return
    from app.models import Meal, MealType, ProteinInventory
    db = SessionLocal()
    try:
        ...
        for m in DEMO_MEALS:
            ...
            db.add(Meal(**data))
        for protein_name, qty in DEMO_PROTEIN_QUANTITIES.items():
            row = db.query(ProteinInventory).filter(
                ProteinInventory.protein_name == protein_name
            ).first()
            if row:
                row.quantity = qty
        db.commit()
