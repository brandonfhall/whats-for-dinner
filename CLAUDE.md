...
7. **Documentation**: Update `docs/ARCHITECTURE.md` (schema, endpoints, features — this is the deep technical reference). `README.md` and `DOCKERHUB.md` are user-facing high-level overviews; do not add per-feature implementation detail or UI call-outs to those files. `ARCHITECTURE.md` is a list of facts and decisions, not a collection of scar tissue about every issue we've had — state current behavior tersely; don't narrate the bug or incident that motivated it (that belongs in the commit message / PR description)
